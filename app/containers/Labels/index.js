...
const Labels = () => {
    ...
    const addedCallsColumns = [
        ...
        },
        {
            ...
            render: (x) => (
                <div size="middle">
                    <Button onClick={e => handleRemoveFromOpList(x)} size={'small'}>Remove</Button>
                </div>
            ),
        }
    ]
    ...
    useEffect(() => {
        (async () => {
            const resp = await axios.get('https://damp-garden-93707.herokuapp.com/getcalllist', { headers: { user_id: '24b456' } });
            setCallList(resp.data.data.call_data);
            console.log("Call List: ", resp.data);
        })();
        (async () => {
            const resp = await axios.get('https://damp-garden-93707.herokuapp.com/getlistoflabels', { headers: { user_id: '24b456' } });
    ...
    const handleApply = async () => {
        const resp = await axios.post('https://damp-garden-93707.herokuapp.com/applyLabels',
            {
                "operation": {
                    "callList": selectedRows, "label_ops": opList
                }
    ...
    return (
        <div className="part2">
            <div className="page2_body">
                <Table rowSelection={rowSelection} columns={labelsListColumn} dataSource={labelsList} />
                <div className="operations">
                    <div className="">
                        <Input onChange={handleCallName} type="text" style={{ width: 300 }} prefix="Name: " />
                        <div className='op_type'>
                            <label htmlFor="">Select Operation Type</label>
                            <Select defaultValue='add' onChange={handleOpChange} style={{ width: 200 }}>
                                <Option value="create">Create</Option>
                                <Option value="add">Add</Option>
                                <Option value="remove">Remove</Option>
                            </Select>
                        </div>
                        <Button type='primary' onClick={handleAddToApply}>Add to Apply</Button>
                        <Button type='primary' onClick={handleApply}>Apply</Button>
                    </div>
                </div>
                <div className="">
                    {opList.length ? <h2>Added Calls</h2> : null}
                    {opList.length ? <Table columns={addedCallsColumns} dataSource={opList}/> : null}
                </div>
